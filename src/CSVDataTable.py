...
class Index():
    ...
    def __init__(self, name=None, table_name=None, columns=None, kind=None, loadit=None):

        if loadit:
            logging.debug("Loading an index.")
            self.from_json(table_name)
            logging.debug("Loaded index. name = %s, columns = %s, kind = %s, table = %s",
                          self.name, str(self.columns), self.kind, table_name)
        else:
            logging.debug("Creating index. name = %s, columns = %s, kind = %s, table = %s",
                          name, str(columns), kind, table_name)
            columns.sort()
            self.name = name
...
class CSVDataTable():
    ...
    _default_directory = "/Users/wangxinquan/Desktop/2sem/W4111Database/xw2566_HW3/DB/"
    ...
    def add_index(self, index_name, column_list, kind):
        ...
        self._indexes[index_name] = Index(name=index_name, table_name=self._table_name, columns=column_list, kind=kind)
    ...
    def insert(self, row):
        ...
        result = self._add_row(row)

    def delete_by_template(self, tmp):
